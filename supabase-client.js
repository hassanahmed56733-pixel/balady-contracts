...
(function () {
  ...
  async function importCloudContractsAndPdfs() {
    ...
    try {
      ...
      if (failed || skipped) {
        notify('تم استيراد ' + imported + ' ملف PDF. تعذر تنزيل ' + failed + '، ويوجد ' + skipped + ' عقدًا مرفقها غير موجود في التخزين السحابي أصلًا. يجب الضغط على «مزامنة» من الجهاز المصدر الذي توجد عليه ملفات PDF ثم إعادة الاستيراد من هذا الجهاز.', 'error');
      } else {
        notify('تم استيراد ' + contracts.length + ' عقد و' + imported + ' ملف PDF إلى هذا الجهاز بنجاح.', 'success');
      }
    } catch (error) {
