...
(function () {
  ...
  function renderAccessState() {
    ...
    if (logoutButton) {
      logoutButton.style.display = new URLSearchParams(window.location.search).get('login') === '1' ? 'none' : '';
    }
  ...
  function showLoginScreen() {
    ...
    const wantsLogin = new URLSearchParams(window.location.search).get('admin-login') === '1';
    if (wantsLogin) {
      if (denied) denied.style.display = 'none';
      if (overlay) { overlay.classList.add('show'); overlay.style.display = 'flex'; }
    } else {
      if (overlay) { overlay.classList.remove('show'); overlay.style.display = 'none'; }
      const screen = denied || document.createElement('div');
      if (!denied) {
        screen.id = 'accessDeniedScreen';
        screen.dir = 'rtl';
        screen.style.cssText = 'position:fixed;inset:0;z-index:99999;background:#0f766e;color:#fff;display:flex;align-items:center;justify-content:center;text-align:center;padding:24px;font-family:Tahoma,sans-serif;';
        screen.innerHTML = '<div style="max-width:420px;background:rgba(0,0,0,.16);border:1px solid rgba(255,255,255,.3);border-radius:16px;padding:28px 22px;"><div style="font-size:42px;margin-bottom:12px;">🔒</div><h2 style="margin:0;">الرابط تالف</h2></div>';
  ...
  async function loadProfile() {
    ...
    if (result.error) console.warn('Profile lookup by id failed:', result.error.message);
    // بعض الحسابات القديمة قد يكون سجلها محفوظًا بالبريد نفسه مع اختلاف معرف الربط.
    ...
    if (result.error) {
      console.warn('Profile lookup by email failed:', result.error.message);
      currentProfile = null;
      return null;
    }
    // لا يُسمح بفتح الواجهة إلا لحساب موجود فعليًا وبريد مطابق في قائمة المستخدمين.
    if (!result.data || String(result.data.email || '').trim().toLowerCase() !== email) {
      currentProfile = null;
      return null;
    }
    currentProfile = result.data;
  ...
  async function syncContractsNow() {
    ...
    // مزامنة تزايدية: لا نعيد إرسال العقد الذي ثبتت مزامنته، وبحد أقصى 100 عقد في الدورة.
    const candidates = contracts.filter(function (contract) { return !contract.__cloudSynced; });
    ...
    persistLocalContracts();
    if (contracts.some(function (contract) { return !contract.__cloudSynced; })) {
      setTimeout(function () { if (session) syncContractsNow().catch(function (error) { console.warn(error); }); }, 900);
    }
    return { ok: true, count: saved.size };
  }

    async function syncLocalPdfs() {
    if (!requireAdmin() || typeof legacyGetPdfRecord !== 'function') return { ok: true, count: 0, failed: 0, eligible: 0 };
    const contracts = Array.isArray(window.allContracts) ? window.allContracts : [];
    let uploadedCount = 0;
    let failedCount = 0;
    let eligibleCount = 0;
    // لا نرفع أكثر من 100 ملف في دورة واحدة، ولا نعيد فحص العقود التي لها مسار سحابي.
    const pendingContracts = contracts.filter(function (contract) {
      return normalizeContractNumber(contract && contract.contractNumber) && !(contract && contract.storagePath);
    });
    for (const contract of pendingContracts) {
      if (eligibleCount >= 100) break;
      ...
      try {
        ...
        for (let attempt = 1; attempt <= 3 && !uploaded; attempt += 1) {
          try {
            uploaded = await uploadPdf(number, file);
          } catch (error) {
            lastError = error;
            console.warn('PDF upload attempt failed', number, attempt, error);
          }
          if (!uploaded && attempt < 3) await new Promise(function (resolve) { setTimeout(resolve, 500 * attempt); });
        }
        if (uploaded) {
          ...
          failedCount += 1;
          console.warn('Local PDF migration failed', number, lastError || 'upload returned no result');
        }
        if ((uploadedCount + failedCount) % 10 === 0) {
          notify('تم رفع ' + uploadedCount + ' من ' + eligibleCount + ' ملف PDF حتى الآن...', 'info');
        }
      } catch (error) {
        failedCount += 1;
        console.warn('Local PDF migration failed', number, error);
      }
    }
    if (uploadedCount) persistLocalContracts();
    // إذا بقيت ملفات بعد حد 100، ابدأ الدفعة التالية تلقائيًا دون تدخل المستخدم.
    if (eligibleCount >= 100 && pendingContracts.length > 100) {
      notify('اكتملت دفعة 100 ملف PDF، وستبدأ الدفعة التالية تلقائيًا...', 'info');
      setTimeout(function () {
        if (session && isCurrentUserAdmin()) {
          syncLocalPdfs().then(function () { return syncContractsNow(); }).catch(function (error) { console.warn('Next PDF sync batch failed', error); });
        }
      }, 1200);
    }
    return { ok: failedCount === 0, count: uploadedCount, failed: failedCount, eligible: eligibleCount };
  }
  async function forceSyncLocalContracts() {
    ...
    notify('جاري رفع العقود وملفات PDF إلى Supabase. قد يستغرق الرفع وقتًا حسب حجم الملفات وسرعة الإنترنت...', 'info');
    ...
    if (result && result.ok) {
      ...
      if (pdfResult.failed > 0) {
        notify('تمت مزامنة ' + result.count + ' عقدًا، لكن فشل رفع ' + pdfResult.failed + ' من ' + pdfResult.eligible + ' ملف PDF. أعد الضغط على «مزامنة» لإعادة المحاولة.', 'error');
      } else {
        notify('تمت مزامنة ' + result.count + ' عقد و' + pdfResult.count + ' ملف PDF والتحقق من مساراتها. افتح الجوال الآن.', 'success');
      }
    }
  }
  ...
  async function importCloudContractsAndPdfs() {
    if (!requireSession() || window.__cloudImportRunning) return;
    window.__cloudImportRunning = true;
    ...
    try {
      ...
      await loadCloudContracts({ keepPage: true });
      const contracts = Array.isArray(window.allContracts) ? window.allContracts : [];
      const allImportCandidates = [];
      for (const candidate of contracts) {
        const candidateNumber = normalizeContractNumber(candidate && candidate.contractNumber);
        const candidatePath = candidate && (candidate.storagePath || candidate.__storagePath);
        if (!candidateNumber || !candidatePath) continue;
        let existingLocal = null;
        try {
          if (typeof legacyGetPdfRecord === 'function') existingLocal = await legacyGetPdfRecord(candidateNumber);
        } catch (eExisting) { console.warn('Local PDF check failed', candidateNumber, eExisting); }
        if (!(existingLocal && existingLocal.blob && existingLocal.blob.size > 0)) allImportCandidates.push(candidate);
      }
      const importCandidates = allImportCandidates.slice(0, 100);
      let eligible = 0, imported = 0, failed = 0, skipped = 0;
      for (const contract of importCandidates) {
        const number = normalizeContractNumber(contract && contract.contractNumber);
      ...
      if (imported) {
        try { if (typeof window.persistLocalContracts === 'function') window.persistLocalContracts(); else localStorage.setItem(LOCAL_CONTRACTS_KEY, JSON.stringify(window.allContracts || [])); } catch (e) {}
      }
      if (failed || skipped) {
        notify('تم استيراد ' + imported + ' ملف PDF. تعذر تنزيل ' + failed + '، ويوجد ' + skipped + ' عقدًا مرفقها غير موجود في التخزين السحابي أصلًا.', 'error');
      } else if (imported || !allImportCandidates.length) {
        notify('تم استيراد ' + imported + ' ملف PDF إلى هذا الجهاز بنجاح.', 'success');
      }
      if (allImportCandidates.length > importCandidates.length) {
        notify('تبقى ' + (allImportCandidates.length - importCandidates.length) + ' ملفًا، وستبدأ الدفعة التالية تلقائيًا.', 'info');
        setTimeout(function () {
          if (session) importCloudContractsAndPdfs().catch(function (error) { console.warn('Next cloud import batch failed', error); });
        }, 1200);
      }
    } catch (error) {
      ...
      window.__cloudImportRunning = false;
  ...
  function scheduleCloudUpload() {
    ...
    cloudUploadTimer = setTimeout(async function () {
      cloudUploadTimer = null;
      if (!session || !isCurrentUserAdmin()) return;
      try {
        const pdfResult = await syncLocalPdfs();
        const contractResult = await syncContractsNow();
        if (pdfResult && (pdfResult.count || pdfResult.failed)) {
          notify('تم رفع ' + pdfResult.count + ' ملف PDF تلقائيًا' + (pdfResult.failed ? '، وتعذر رفع ' + pdfResult.failed + ' ملف.' : '.'), pdfResult.failed ? 'error' : 'success');
        }
        if (contractResult && !contractResult.ok) console.warn('Automatic contract sync failed', contractResult.error);
      } catch (error) { console.warn('Automatic cloud upload failed', error); }
    }, 900);
  }
  ...
  async function getSharedPdfRecord(contractNumber) {
    ...
    if (!number || !requireSession()) return null;
    let contract = (window.allContracts || []).find(function (item) { return String(item.contractNumber) === number; });
    let path = contract && (contract.storagePath || contract.__storagePath);
    let fileName = contract && contract.fileName;
    // إذا لم يكن مسار الملف موجودًا في الذاكرة المحلية، ابحث عنه مباشرة في السحابة قبل فتح منتقي الهاتف.
    if (!path) {
      try {
        const rowResult = await client.from(CONTRACTS_TABLE)
          .select('contract_number,file_name,storage_path')
        ...
        if (!rowResult.error && rowResult.data && rowResult.data[0]) {
          ...
          if (contract && path) {
            contract.storagePath = path;
            contract.fileName = fileName || contract.fileName;
          }
        }
      } catch (error) { console.warn('Cloud PDF path lookup failed:', error); }
    }
    if (!path) return null;
    const result = await client.storage.from(FILES_BUCKET).download(path);
    if (result.error || !result.data || result.data.size === 0) {
      console.warn('Shared PDF download failed:', result.error && result.error.message);
      return null;
    }
    return { contractNumber: number, blob: result.data, fileName: fileName || (number + '.pdf'), saved: true, shared: true };
  }
  ...
  function isSessionExpired() {
    ...
    // جلسة Supabase المحفوظة صالحة حتى لو لم توجد علامة الوقت من نسخة قديمة؛ تُسجّل عند استعادتها.
    if (!at) return false;
  ...
  async function logoutApp() {
    if (!window.confirm('هل تريد إخفاء الواجهة؟ سيبقى هذا الجهاز مصرحًا به.')) return;
    // إخفاء الواجهة فقط؛ لا نلغي جلسة Supabase حتى يبقى الجهاز المصرح به قادرًا على العودة.
    showLoginScreen();
    notify('تم إخفاء الواجهة. يمكنك العودة إليها من هذا الجهاز المصرح به.', 'success');
  }
  ...
  function installOverrides() {
    ...
    window.saveContractToCloud = function () { scheduleSync(); scheduleCloudUpload(); };
    ...
    window.scheduleCloudUpload = scheduleCloudUpload;
  ...
  async function onSignedIn(options) {
    ...
    if (!profile || String(profile.email || '').trim().toLowerCase() !== String(session.user.email || '').trim().toLowerCase()) {
      try { await client.auth.signOut(); } catch (e) {}
      session = null;
      currentProfile = null;
      showLoginScreen();
      const errorBox = document.getElementById('loginError');
      if (errorBox) { errorBox.textContent = 'هذا البريد غير مضاف من المدير أو غير مصرح له بفتح الواجهة.'; errorBox.classList.add('show'); }
      return;
    }
    showAppUnlocked();
    startSessionWatch();
    ...
    // تنزيل المرفقات الناقصة تلقائيًا بعد فتح الجهاز، دون إعادة تنزيل الملفات الموجودة محليًا.
    setTimeout(function () {
      if (session) importCloudContractsAndPdfs().catch(function (error) { console.warn('Automatic cloud import failed', error); });
    }, 900);
    installRealtime();
