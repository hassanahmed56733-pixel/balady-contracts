...
(function () {
  ...
  function showLoginScreen() {
    ...
    if (wantsLogin) {
      ...
      if (!denied) {
        ...
        screen.innerHTML = '<div style="max-width:420px;background:rgba(0,0,0,.16);border:1px solid rgba(255,255,255,.3);border-radius:16px;padding:28px 22px;"><div style="font-size:42px;margin-bottom:12px;">🔒</div><h2 style="margin:0 0 10px;">الرابط غير صالح أو غير مصرح باستخدامه</h2><p style="margin:0;line-height:1.8;">لا يملك هذا المتصفح جلسة دخول لحساب مضاف من المدير.</p></div>';
  ...
  async function loadProfile() {
    ...
    // لا يُسمح بفتح الواجهة إلا لحساب موجود فعليًا في قائمة المستخدمين.
    if (!result.data || String(result.data.id) !== String(session.user.id)) {
      currentProfile = null;
      return null;
    }
