...
(function () {
  ...
  function renderAccessState() {
    ...
    if (userButton) {
      userButton.textContent = isCurrentUserAdmin() ? 'صلاحيات المدير' : '';
  ...
  async function syncContractsNow() {
    if (!requireSession() || cloudApplying) return;
  ...
  async function uploadPdf(contractNumber, file) {
    ...
    const safeName = String(file.name || 'contract.pdf').replace(/[^\w.\-\u0600-\u06FF]/g, '_');
    const safeNumber = encodeURIComponent(normalizeContractNumber(contractNumber) || 'unassigned');
    const path = session.user.id + '/' + safeNumber + '/' + Date.now() + '-' + safeName;
    ...
    return { path: result.data.path, fileName: safeName };
  }
