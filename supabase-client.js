...
(function () {
  ...
  const DEVICE_AUTHORIZED_KEY = 'baladyDeviceAuthorizedV1';
  ...
  function isDeviceAuthorized() {
    try { return localStorage.getItem(DEVICE_AUTHORIZED_KEY) === '1'; } catch (e) { return false; }
  }
  function markDeviceAuthorized() {
    try { localStorage.setItem(DEVICE_AUTHORIZED_KEY, '1'); } catch (e) {}
  }
  ...
  function notify(message, type) {
    ...
    if (status && message) {
      ...
      if (type === 'success') {
        syncStatusHideTimer = setTimeout(function () {
  ...
  function rowToContract(row) {
    ...
    data.__cloudUpdatedAt = data.updatedAt;
  ...
  function renderAccessState() {
    ...
    if (logoutButton) logoutButton.style.display = isAdminLoginRoute() ? 'none' : 'inline-flex';
  ...
  function showLoginScreen() {
    if (!session && !isAdminLoginRoute() && !isDeviceAuthorized()) { showCorruptLinkScreen(); return; }
    const corrupt = document.getElementById('corruptLinkOverlay');
    if (corrupt) corrupt.style.display = 'none';
    const overlay = document.getElementById('loginOverlay');
    const app = document.getElementById('appRoot');
  ...
  async function loadCloudContracts(options) {
    ...
    try {
      ...
      const pending = (window.allContracts || []).filter(function (contract) {
        return !contract.__cloudSynced || (contract.__cloudUpdatedAt && Number(contract.updatedAt || 0) > Number(contract.__cloudUpdatedAt || 0));
      });
  ...
  async function syncContractsNow() {
    ...
    // لا تعِد رفع العقود السحابية المكتملة؛ المدير يرفع فقط الجديد أو المعدل.
    ...
    contracts.forEach(function (contract) {
      if (saved.has(String(contract.contractNumber))) {
        contract.__cloudSynced = true;
        contract.__cloudUpdatedAt = Number(contract.updatedAt || Date.now());
      }
    });
    persistLocalContracts();
  ...
  async function attemptLogin(event) {
    ...
    // اعتماد الجهاز بعد أول تسجيل دخول ناجح؛ بعد ذلك يكفي الرابط الرئيسي.
  ...
  async function deleteContract(contractNumber) {
    ...
    persistLocalContracts();
    // الحفاظ على الصفحة الحالية بعد وصول تأكيد الحذف من السحابة.
    if (typeof window.applyFilters === 'function') window.applyFilters(true);
  ...
  async function deleteContractsFromCloud(numbers) {
    if (!requireAdmin()) return false;
    const list = Array.from(new Set((numbers || []).map(function (n) { return normalizeContractNumber(n); }).filter(Boolean)));
    if (!list.length) return true;
    const rows = await client.from(CONTRACTS_TABLE).select('contract_number,storage_path').in('contract_number', list);
    if (rows.error) throw new Error(rows.error.message);
    const paths = (rows.data || []).map(function (row) { return row.storage_path; }).filter(Boolean);
    if (paths.length) {
      const removed = await client.storage.from(FILES_BUCKET).remove(paths);
      if (removed.error) throw new Error(removed.error.message);
    }
    const result = await client.from(CONTRACTS_TABLE).delete().in('contract_number', list);
  ...
  function installOverrides() {
    ...
    window.deleteContractsFromCloud = deleteContractsFromCloud;
