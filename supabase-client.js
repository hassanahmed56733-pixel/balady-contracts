...
(function () {
  ...
  async function importCloudContractsAndPdfs() {
    ...
    try {
      ...
      await loadCloudContracts({ keepPage: true });
      const contracts = Array.isArray(window.allContracts) ? window.allContracts : [];
      const allImportCandidates = contracts.filter(function (contract) {
        return normalizeContractNumber(contract && contract.contractNumber) && Boolean(contract && (contract.storagePath || contract.__storagePath));
      });
      ...
      for (const contract of importCandidates) {
        ...
        if (typeof legacyGetPdfRecord === 'function') {
          const existing = await legacyGetPdfRecord(number);
          if (existing && existing.blob && existing.blob.size > 0) continue;
        }
        eligible += 1;
        let blob = null;
